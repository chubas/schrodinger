...
function displayRunComparison(runs) {
  ...
  const rows = runs.map(run => {
    ...
    if (run.results.memory) {
      ...
      // Use average memory if available
      const avgMemory = run.results.memory.average ?
        formatBytes(run.results.memory.average.heapUsed) :
        'N/A';

      const endMemory = formatBytes(run.results.memory.diff.heapUsed);

      row.push(peakMemory, avgMemory, endMemory);
    } else {
...
function displayTrends(runs) {
  ...
  const timeDetails = ` (${first.results.executionTime.toFixed(2)}ms → ${last.results.executionTime.toFixed(2)}ms)`;

  // Calculate iterations change if available
  let collapsesChange = percentChange(first.results.collapses, last.results.collapses);
  let backtracksChange = percentChange(first.results.backtracks, last.results.backtracks);

  // Calculate snapshots change if available
  ...
  if (firstHasMemory && lastHasMemory) {
    ...
    // For average memory
    if (first.results.memory.average && last.results.memory.average) {
      avgMemoryChange = percentChange(
        first.results.memory.average.heapUsed,
        last.results.memory.average.heapUsed
      );
      avgMemoryDetails = ` (${formatBytes(first.results.memory.average.heapUsed)} → ${formatBytes(last.results.memory.average.heapUsed)})`;
    }
  ...
  // Display trends
  console.log('\nTrends:');
  console.log(`Execution time: ${timeChange}${timeDetails}`);
  ...
  if (firstHasMemory || lastHasMemory) {
    ...
    if (avgMemoryChange !== 'N/A') {
      console.log(`Average memory usage: ${avgMemoryChange}${avgMemoryDetails}`);
    }
