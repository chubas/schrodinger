...
describe("WFC", () => {
  ...
  describe("RNG Integration", () => {
    ...
    it("should use provided RNG sequence", async () => {
      ...
      const sequenceRNG = new RecordingRNG(new DeterministicRNG(sequence));
      ...
      await new Promise<void>((resolve) => {
        ...
        wfc.on("complete", () => {
          ...
          expect(sequenceRNG.draws.length).toBeGreaterThan(0);
          resolve();
        });
      ...
      const replayGrid = new SquareGrid(2, 2);
      const replayWfc = new WFC(mockTiles, replayGrid, {
        random: createReplayRNG(sequenceRNG.draws),
      });
      replayWfc.start();

      for (const [cell, coords] of wfc.iterate()) {
        const replayCell = replayGrid.get(coords)!;
        expect(replayCell.choices.map((tile) => tile.name)).toEqual(
