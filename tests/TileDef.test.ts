...
describe("TileDefFactory", () => {
  ...
  it("should parse adjacencies with tokens", () => {
    const adjacencies = TileDefFactory.extractAdjacencies("(some)(token)|A|(other)(token)");
    expect(adjacencies).toEqual([["some", "token"], ["A"], ["other", "token"]]);
  });

  it("should parse mixed adjacencies with tokens", () => {
    const adjacencies = TileDefFactory.extractAdjacencies("AB(some)CD|(other)EF(token)");
    expect(adjacencies).toEqual([["A", "B", "some", "C", "D"], ["other", "E", "F", "token"]]);
  });
